...
>(({ className, viewport, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Root
    ...
    {...props}
  >
    <div className="flex w-[min(120ch,100%)] items-center justify-stretch overflow-auto p-4 [&>div]:flex-1">
      {children}
    </div>
    <NavigationMenuViewport className={viewport?.className} />
  </NavigationMenuPrimitive.Root>
));
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.List
    ref={ref}
    className={cn(
      "group flex w-full flex-1 list-none items-center justify-start gap-2 space-x-1",
      className,
    )}
    {...props}
  />
...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-10 gap-2 w-max items-center justify-center rounded-md bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent data-[state=open]:bg-accent",
);
...
>(({ children, variant, ...props }, ref) => (
  <NavigationMenuPrimitive.Link ref={ref} asChild>
    <Button variant={variant ?? "ghost"} {...props}>
      {children}
    </Button>
