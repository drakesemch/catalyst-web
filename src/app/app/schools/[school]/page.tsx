        ...
        <div className="h1 flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <h2 className="text-lg font-medium text-muted-foreground">
              {school?.district ?? "No District"}
            </h2>
          </div>
        </div>
        <div className="flex h-auto gap-2 overflow-auto">
          <Button
            variant="outline"
            className="h-8 rounded-full px-4 py-1 text-xs text-muted-foreground"
            href={`https://www.google.com/maps/place/${school?.address} ${school?.city} ${school?.state}`}
            target="_blank"
          >
            <MapPin /> {school?.address}, {school?.city}, {school?.state}
          </Button>
          <Button
            variant="outline"
            className="h-8 rounded-full px-4 py-1 text-xs text-muted-foreground"
            ...
            target="_blank"
          >
            <CircleUserRound /> {school?.canvasURL}
          </Button>
          <Button
            variant="outline"
            className="pointer-events-none h-8 rounded-full px-4 py-1 text-xs text-muted-foreground"
          >
            <School /> {school?.name?.split(" ").slice(-2).join(" ")}
          </Button>
          {permissions.find(
            (p) => p.role == "owner" || p.role == "admin" || p.role == "helper",
          ) && (
            <Button
              variant="secondary"
              className="h-8 rounded-full px-4 py-1 text-xs text-muted-foreground"
              href={`/app/schools/${schoolId}/modify/dates`}
            >
              <CalendarPlus /> Add Schedule to Dates
            </Button>
          )}
          {permissions.find((p) => p.role == "owner" || p.role == "admin") && (
            <Button
              variant="secondary"
              className="h-8 rounded-full px-4 py-1 text-xs text-muted-foreground"
                ...
                {schedule.periods.map((period) => (
                  ...
                  >
                    <div className="flex items-stretch">
                      <div className="h-auto flex-1 overflow-hidden px-4 py-2">
