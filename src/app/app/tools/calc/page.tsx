...
export default function CalculatorPage() {
  ...
  const [precisionModes, setPrecisionModes] = useState<PrecisionMode[]>([
    "exact",
  ]);
  const [fractionsModes, setFractionsModes] = useState<FractionMode[]>([
    "improper",
  ]);
  const [defaultPrecisionMode, setDefaultPrecisionMode] =
    useState<PrecisionMode>("exact");
  const [defaultFractionsMode, setDefaultFractionsMode] =
    useState<FractionMode>("improper");
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      expressions.forEach((expression, index) => {
        ...
        const degreeRegex =
          /(\b(sin|cos|tan|asin|acos|atan)\()\s*([0-9A-Za-z.]+)\s*(\))/g;
        ...
        if (expression.includes("=")) {
          ...
          if (lhs && rhs) {
            try {
              ...
              ce!.assign(
                lhs,
                isFunction ? ["Function", result.toMathJson()] : result,
              );
              ...
              if (fractionMode == "mixed") {
                try {
                  const mixed = new Fraction(
                    (result.toMathJson() as unknown as number[])[1]!,
                    (result.toMathJson() as unknown as number[])[2]!,
                  ).toFraction(true);
                  ...
                  try {
                    ...
                    function replaceFraction(
                      ...
                      if (result[0] === "Rational") {
                        ...
                        replacements[`${result[1]!}/${result[2]!}`] =
                          new Fraction(Number(result[1]))
                            .div(Number(result[2]))
                            .toFraction(true);
                      }
    ...
    fractionsModes,
    trigEvalMode,
    defaultPrecisionMode,
    defaultFractionsMode,
  ]);
    ...
    <>
      <main className="mx-auto flex max-w-[100ch] flex-1 flex-shrink flex-col gap-2 overflow-auto p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex origin-right scale-75 items-center gap-4">
            <Drawer>
              <DrawerTrigger asChild>
                <Button variant="secondary" className="-mr-24">
                  <HelpCircle />
                  Help
                </Button>
              </DrawerTrigger>
              <DrawerContent>
                <DrawerHeader>
                  <DrawerTitle>Calculator Info</DrawerTitle>
                </DrawerHeader>
                <div className="p-4 pt-0">
                  <p>
                    Our calculator supports a variety of mathematical
                    expressions, including those using LaTeX (for example using
                    pi, you would enter <code className="code">\pi</code>). You
                    can also define and use functions and variables within your
                    calculations. For example:
                  </p>
                  <ul>
                    <li>
                      Basic arithmetic: <code className="code">2 + 2</code>
                    </li>
                    <li>
                      Using LaTeX: <code className="code">\pi + 3</code> or{" "}
                      <code className="code">\frac{"{3}{2}"}</code>
                    </li>
                    <li>
                      Defining a function:{" "}
                      <code className="code">f(x) = x^2</code>, then{" "}
                      <code className="code">f(3)</code>
                    </li>
                    <li>
                      Using variables: <code className="code">a = 5</code>, then{" "}
                      <code className="code">a + 3</code>
                    </li>
                  </ul>
                  <p>
                    Switch between degrees and radians for trigonometric
                    functions using the DEG/RAD toggle.
                  </p>
                </div>
              </DrawerContent>
            </Drawer>
            <Tabs
              value={trigEvalMode}
              onClick={() =>
                setTrigEvalMode(trigEvalMode == "deg" ? "rad" : "deg")
              }
              className="w-max"
            >
              <TabsList>
...
function Expression({
  ...
  const setFractionMode = () => {
    ...
    if (isLast) {
      setDefaultFractionsMode((prev) =>
        prev == "improper" ? "mixed" : "improper",
      );
    }
  };
  ...
  result =
    result == '"Nothing"'
      ...
      : result
          ...
          .replace(
            /root\((\d+)\)\((\d+)\)/g,
            (_, base, radicand) => `\\sqrt[${base}]{${radicand}}`,
          )
  ...
  return (
    <div className="flex flex-col items-center gap-2 border-b pb-4 md:flex-row md:border-0">
      <div className="flex w-full flex-1 flex-row gap-2">
            ...
            <DropdownMenuItem
              onClick={() => {
                setExpressions((prev) => {
                  ...
                  [newExpressions[idx - 1], newExpressions[idx]] = [
                    newExpressions[idx],
                    newExpressions[idx - 1]!,
                  ];
                  return newExpressions;
                });
                setPrecisionModes((prev) => {
        ...
        <label className="group relative ml-4 h-12 w-full flex-1 text-2xl">
          <Input
            value={value}
            onChange={handleChange}
            key={`expr-inp-${idx}`}
            className="absolute inset-0 h-10 flex-1 border-0 px-4 py-6 font-serif text-2xl opacity-0 group-focus-within:opacity-100 group-active:opacity-100"
          />
          {value == "" ? (
            <div className="relative left-0 top-0 flex h-10 cursor-text px-4 py-2 font-serif text-muted-foreground group-focus-within:opacity-0 group-active:opacity-0">
              Enter an expression
            </div>
          ) : (
            <div className="relative left-0 top-0 flex h-10 cursor-text px-4 py-2 group-focus-within:opacity-0 group-active:opacity-0">
      ...
      <div className="ml-auto flex gap-4">
        {result && (
          <span className="flex w-full justify-end px-2 text-end font-serif text-2xl text-accent-foreground">
        ...
        <div className="ml-4 flex flex-col items-end justify-center">
          <button
            ...
            onClick={setPrecisionMode}
          >
            <div className="flex gap-2 text-[0.5rem]">
              <span
                className={exact ? "text-primary" : "text-muted-foreground"}
              >
                EXACT
              </span>
              <span
                className={approx ? "text-primary" : "text-muted-foreground"}
              >
                APPROX
              </span>
            </div>
          ...
          <button
            className="flex w-full cursor-pointer flex-col items-end"
            disabled={!exact}
            onClick={setFractionMode}
          >
            <div className="flex gap-2 text-[0.5rem]">
              <span
                className={
                  exact && improper ? "text-primary" : "text-muted-foreground"
                }
              >
                IMPROPER
              </span>
              <span
                className={
                  exact && mixed ? "text-primary" : "text-muted-foreground"
                }
              >
                MIXED
              </span>
